...
#   shannon_entropy    Shannon entropy H, natural log (in nats, not in bits)
...
def calculate_kmer_metrics_chunk(kmer_df: pd.DataFrame) -> pd.DataFrame:
    ...
    # Shannon entropy in nats (natural log, not log2 like in bits)
    shannon_entropy = -plogp.sum(axis=1, dtype=np.float64)
    ...
    pielou_evenness = np.where(
        num_nonzero > 1,
        shannon_entropy / np.log(np.maximum(num_nonzero, 2)),
        1.0,
    )
...
def process_kmer_file(
    ...
    def write_metrics(metrics: pd.DataFrame):
        ...
        entropy_stats.update(
            metrics["shannon_entropy"].to_numpy()
        )
    ...
    info(
        f"{'shannon entropy':<{w}}  "
        f"{'mean':<4} {entropy_stats.mean:8.3f}  "
        f"{'sd':<3} {entropy_stats.std(ddof=1):8.3f}"
    )

    info(
        f"{'shannon range':<{w}}  "
        f"{'min':<4} {entropy_stats.min:8.3f}  "
        f"{'max':<3} {entropy_stats.max:8.3f}"
    )
